...
/**
 * Service is responsible for managing game state requests from the {@link com.hexanome16.server.controllers.GameController}.
 */
@Service
public class GameService {
   ...
   */
  public DeferredResult<ResponseEntity<String>> getDeck(long sessionId,
                                                        String level,
                                                        String accessToken,
                                                        String hash) {
   ...
   */
  public DeferredResult<ResponseEntity<String>> getCurrentPlayer(long sessionId,
                                                                 String accessToken,
                                                                 String hash) {
    if (authService.verifyPlayer(sessionId, accessToken, gameMap)) {
      DeferredResult<ResponseEntity<String>> result;
   ...
   */
  public ResponseEntity<String> getPlayerBankInfo(long sessionId,
                                                  String username)
    ...
    return new ResponseEntity<String>(objectMapper.writeValueAsString(playerBankMap),
        HttpStatus.OK);
  }
   ...
   * @throws JsonProcessingException if Json processing fails
   */
  public ResponseEntity<String> getGameBankInfo(long sessionId)
      throws JsonProcessingException {
    // session not found
    if (!gameMap.containsKey(sessionId)) {
      return new ResponseEntity<String>(HttpStatus.BAD_REQUEST);
    ...
    return new ResponseEntity<String>(objectMapper.writeValueAsString(gameBankMap), HttpStatus.OK);
  }
   ...
   */
  public ResponseEntity<String> buyCard(long sessionId,
                                        String cardMd5,
